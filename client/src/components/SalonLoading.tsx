    ...
    <div className="salon-root" style={{ alignItems: 'center', justifyContent: 'center', display: 'flex' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 28, zIndex: 5 }}>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
          <span style={{ fontSize: 11, letterSpacing: '0.32em', textTransform: 'uppercase', color: 'var(--brass-soft)', fontWeight: 600 }}>
            Maison · de · Belote
          </span>
          <h1 style={{ margin: 0, fontFamily: "'Fraunces', serif", fontSize: 56, fontWeight: 500, color: 'var(--ink)', letterSpacing: '-0.01em' }}>
            Belote
          </h1>
        </div>
        ...
        <div style={{ display: 'flex', gap: 14, padding: '8px 0' }}>
          {(['hearts', 'spades', 'diamonds'] as const).map((s, i) => (
            <div key={s} style={{
              ...
              <div style={{ position: 'absolute', inset: 4, border: '1px solid rgba(201,162,75,0.55)', borderRadius: 5 }} />
              <span style={{ fontFamily: "'Fraunces', serif", fontStyle: 'italic', color: 'var(--brass)', fontSize: 22, position: 'relative', zIndex: 1 }}>B</span>
            </div>
          ))}
        </div>

        <p style={{ fontSize: 12, letterSpacing: '0.18em', textTransform: 'uppercase', color: 'var(--ink-soft)', fontWeight: 600, margin: 0 }}>
          {label}
